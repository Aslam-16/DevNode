...
router.post('/connectionrequest/:status/:userId', userauth,async(req,res)=>{
    ...
    try{
        ...
        if(!ALLOWED_STATUS.includes(status)) return res.status(400).json({success:false,message:"invalid status "+status});
        if(!mongoose.Types.ObjectId.isValid(userId)) return res.status(400).json({success:false,message:"invalid user id format"});
        if(userId===req.user._id.toString()) return res.status(400).json({success:false,message:"you cannot send connection request to yourself"});
        const isuserexist=await User.findById(userId);
        console.log("isuserexist",isuserexist);
        if(!isuserexist) return res.status(404).json({success:false,message:"user not found with id "+userId});
        ...
        if(isrequestalreadyexist) return res.status(400).json({success:false,message:"request already exist with this user"});
...
router.post('/respondtorequest/:requestId/:status', userauth,async(req,res)=>{
    try {
        ...
        if(!ALLOWED_STATUS.includes(status)) return res.status(400).json({success:false,message:"invalid status "+status});
        if(!mongoose.Types.ObjectId.isValid(requestId)) return res.status(400).json({success:false,message:"invalid request id format"});
        const requestInfo=await Request.findOne({_id:requestId,status:"interested"});
        if(!requestInfo) return res.status(404).json({success:false,message:"request not found with id "+requestId});
        if(requestInfo.toUserId.toString()!==req.user._id.toString()) return res.status(403).json({success:false,message:"you are not authorized to update this request"});
        if(requestInfo.status===status) return res.status(400).json({success:false,message:"request is already updated to  "+status});
...
router.get('/connection/requestreceived', userauth,async(req,res)=>{
    try {
        ...
        if(requests.length===0) return res.json({success:false,message:"no requests found",requests:[]});
...
router.get('/connections', userauth,async(req,res)=>{
    try{
        ...
        if(allConnections.length===0) return res.json({success:false,message:"no connections found",connections:[]});
